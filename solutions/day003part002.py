        ...
        for x in range(-1, 2):
            for y in range(-1, 2):
                if x == 0 and y == 0:
                    continue
                adjacent_index = [
                    start_index[0] + x, start_index[1] + y]
...
symbol_adjacent_indexes = deduplicate_indexes(
    ...
    for gear_index in gear_indexes:
        ...
        for part_number in part_numbers:
            ...
            if is_part_number_adjacent:
                adjacent_part_number_values.append(
                    part_number["value"])

        if len(adjacent_part_number_values) == 2:
            ratios.append(
                adjacent_part_number_values[0] * adjacent_part_number_values[1]
            )

    return ratios


gear_ratios = find_gear_ratios(
    part_numbers, gear_indexes)
